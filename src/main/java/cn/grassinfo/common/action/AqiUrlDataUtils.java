 ...
 * @date 2015-08-18
 */
public class AqiUrlDataUtils {

	private static final Logger log = LoggerFactory.getLogger(AqiUrlDataUtils.class);
	private static final SimpleDateFormat df3 = new SimpleDateFormat("yyyyMMddHH");

	/**
	 * 发送文件
	 */
	private static void sendFile(String localFilePath, String suffixPath) {
		try {
			File file = new File(localFilePath);
			if (file.exists()) {
				// FtpDataSendUtils.sendSingleFile(file.getPath(),file.getName(),
				// Constant.ftpFileManager.getString("weatherIndexReceivePath")+suffixPath);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	public static void createAqiData() {
		...
		ResultSet rs = null;
		try {
			String oldYH = Constant.propFileManager.getString("oldYHStation");
			if (StringUtils.isEmpty(oldYH)) {
				oldYH = "k1207";
			}

			String defaultYHStation = Constant.propFileManager.getString("defaultYHStation");

			if (StringUtils.isEmpty(defaultYHStation)) {
				defaultYHStation = "58444";
			}

			String sql = "SELECT DISTINCT(i.stationN),i.region from zcy.info_town i";
			ps = conn.prepareStatement(sql.toString());
			rs = ps.executeQuery();
			Map<String, String> maps = new HashMap<String, String>();
			while (rs.next()) {
				String stationN = rs.getString("stationN").trim();
				String region = rs.getString("region").trim();
				if (stationN.equals(oldYH)) {
					stationN = defaultYHStation;
				}
				maps.put(stationN, region);
			}
			...
			List<StationAqi> sas = new ArrayList<StationAqi>();
			String StationName = null;
			if (jsonArray != null && jsonArray.size() > 0) {
				for (int i = 0; i < jsonArray.size(); i++) {
					com.alibaba.fastjson.JSONObject jsonObject = jsonArray.getJSONObject(i);
					String station = jsonObject.getString("Station");
					String country = "";// 默认的情况，只有这个找不到站点对应关系
					if (station != null && !"".equals(station)) {
						country = maps.get(station);
					}
					...
					if (jsonArrayChildren != null && jsonArrayChildren.size() > 0 && flag) {
						for (int y = 0; y < jsonArrayChildren.size(); y++) {
							...
							StationName = jsonObjectChild.getString("StationName");
							StationAqi sa = new StationAqi();
							sa.setCountry(StringUtils.isEmpty(country) ? null : country);
							sa.setName(StationName);
			...
			String jsonText = com.alibaba.fastjson.JSON.toJSONString(sas);
			List<File> files = new ArrayList<File>(); 
			if (!StringUtils.isEmpty(jsonText)) {
				String outPutPath = FileUtils.getContextPath() + "aqi/actual_allstation.json";
