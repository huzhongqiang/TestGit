...
/* 单个删除 */
function comfirm_fun(obj, data,remind,secMsg,errMsg) {
    layer.confirm(remind, function () {
        $.post($(obj).data("url"), data, function (rs) {
            if (rs.success) {
                //$(obj).parents("tr").remove();
                layer.msg(secMsg, {icon: 1, time: 1000}, function () {
                    window.location.reload();
                });
            } else {
                if (rs.code == '104') {
                    layer.msg(rs.message, {icon: 2, time: 1000});
                } else {
...
/* 批量删除 */
function comfirm_batch_del(obj, data) {
    layer.confirm('确定要删除所选？', function () {
        if (!$(".check").is(":checked")) {
            layer.msg('至少选择一个删除!', {icon: 3, time: 1000});
            return false;
        }
        $.post($(obj).data("url"), data.serialize(), function (rs) {
            if (rs.success) {
...
/* 禁用、启用 */
function comfirm_disableOrEnable(obj, data) {
    layer.confirm('确定要操作所选？', function () {
        if (!$(".check").is(":checked")) {
            layer.msg('至少选择一个!', {icon: 3, time: 1000});
            return false;
        }
        $.post($(obj).data("url"), data.serialize(), function (rs) {
            if (rs.success) {
